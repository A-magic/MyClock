...
//读取前置超声波测距数据
void read_csb(int EchoPin, int TrigPin)
{
  unsigned int s, t, f;
  ...
  for (s = 0; s < 6; s++)   //连续6次，读取前置超声波测距数值
  {
    ...
    switch (TrigPin)
    {
      case trigpinM:   a[s] = pulseIn(EchoPin, HIGH) / 58.00;   //计算出测距数值，存放于前置超声波数组中
        break;
      case trigpinL:   b[s] = pulseIn(EchoPin, HIGH) / 58.00;
        break;
      case trigpinR:  c[s] = pulseIn(EchoPin, HIGH) / 58.00;
        break;
    }
...
void CJ()
{
  read_csb(echopinM, trigpinM);      //读前置超声波
  read_csb(echopinL, trigpinL);      //读左侧超声波
  read_csb(echopinR, trigpinR );       //读右侧超声波
  Cj_data();      //对数据排序
  JL();           //掐头去尾取平均，得到测距值
}
